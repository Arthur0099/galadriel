...
contract Verifier {
    ...
    function verify(
        ...
        state.refresh.X = ps[8];
        state.refresh.Y = ps[9];
        state.mrct.X1 = ps[2];
        state.mrct.X2 = ps[3];
        state.mrct.Y = ps[4];
        state.custom = new uint256[](12);
        ...
        proof.pteProof.A1 = ps[5];
        proof.pteProof.A2 = ps[6];
        proof.pteProof.B = ps[7];
        proof.pteProof.z1 = scalar[0];
        proof.pteProof.z2 = scalar[1];

        proof.dleProof.A1 = ps[12];
        proof.dleProof.A2 = ps[13];
        proof.dleProof.z = scalar[4];

        proof.ctvProof.A = ps[10];
        proof.ctvProof.B = ps[11];
        proof.ctvProof.z1 = scalar[2];
        proof.ctvProof.z2 = scalar[3];

        proof.aggProof.A = ps[14];
        proof.aggProof.S = ps[15];
        proof.aggProof.T1 = ps[16];
        proof.aggProof.T2 = ps[17];
    ...
    function verifyPTEProof(
        ...
        uint256 e = uint256(
            keccak256(abi.encodePacked(proof.A1.X, proof.A1.Y, proof.A2.X, proof.A2.Y, proof.B.X, proof.B.Y))
        )
            .mod();
