...
contract PGC {
    ...
    event LogAggTransfer(address indexed proxy, address indexed token, uint256[4] fromto, uint256[6] ct, uint256 time);
    event LogTransfer(
        address indexed proxy,
        address indexed token,
        uint256[4] fromto,
   ...
   */
    function aggTransfer(
        ...
        require(points.length == 33 * 18, 'Invalid points length');
