...
// AggRangeParams contains all params to generate/verify aggrated range proof.
...
// NewAggRangeParams returns a new instance of aggrate range proof params.
...
// NewRandomAggRangeParams generates a random params for aggrate range proof.
func NewRandomAggRangeParams(curve elliptic.Curve, bitsize, aggsize int) AggRangeParams {
...
// AggRangeProof aggrates multi range proofs to a single proof.
...
// ToSolidityInput format datas to solidity input to test for contract.
...
// GenerateAggRangeProof aggreates many(2 current) bullet proof together to reduce proof size.
func GenerateAggRangeProof(params AggRangeParams, v, random []*big.Int) (*AggRangeProof, error) {
	...
	if len(v) != aggsize {
		return nil, fmt.Errorf("witness v len %d not equal aggreate size %d", len(v), aggsize)
	}

	if len(random) != aggsize {
		return nil, fmt.Errorf("witness r len %d not equal aggreate size %d", len(random), aggsize)
	}
...
// VerifyAggRangeProof verify agrreate range proofs.
func VerifyAggRangeProof(params AggRangeParams, v []*utils.ECPoint, proof *AggRangeProof) bool {
	...
	dleta := utils.DeltaMN(y, z, n, m, bitsize)
...
// OptimizedVerifyAggRangeProof verify aggrated range proof.
