...
func GenerateCTValidProof(params BaseParams, pk *ecdsa.PublicKey, ct *TwistedELGamalCT) (*CTValidProof, error) {
	...
	// B = g*b + h*a.
	proof.B = new(utils.ECPoint).ScalarMult(params.G(), b)
	tmp := new(utils.ECPoint).ScalarMult(params.H(), a)
...
func VerifyCTValidProof(params BaseParams, pk *ecdsa.PublicKey, ct *CTEncPoint, proof *CTValidProof) bool {
	...
	left = new(utils.ECPoint).ScalarMult(params.G(), proof.Z2)
	tmp := new(utils.ECPoint).ScalarMult(params.H(), proof.Z1)
	left.Add(left, tmp)
